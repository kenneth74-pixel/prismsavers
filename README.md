...
To login to this mariadb docker database, you can use:

    docker exec -it mariadb-10.6 bash

Alternatively, using docker, you can install and run a phpmyadmin interface by running:

    docker run --name kb-phpmyadmin -d --link mariadb-10.6:db -p 8081:80 phpmyadmin/phpmyadmin

You can then access the PHPMyAdmin by going to http://localhost:8081/ and logging in with `root/password`.
...
Due to licensing, Pentaho support was dropped from the project. The Mifos X project continues to support it through an additional class that customises the processing just for Pentaho reports.
We have adopted a similar approach by including:

    org.apache.fineract.infrastructure.report.provider.PentahoReportingProcessServiceImpl

Which is registered as a `ReportingProcessService`. We've also included the required libraries and core engine in the gradle dependencies.

The most recent approach is to use a plugin such as <https://github.com/openMF/fineract-pentaho>, but we don't really gain much from the separation

## Licensing Tenants

There's an external tenant management service that keeps track of how many clients a tenant can use. Setting it up requires the following ENV variables

    TENANT_MANAGER_API - should correspond to the endpoint of the tenant management service that returns license details
    TENANT_MANAGER_KEY - should correspond to the authentication key we send to the tenant management service

To disable the licensing, e.g. because you are working locally and don't want to start the tenant manager, add this to the ENV

    VALIDATE_LICENSE=FALSE

## Environment Configuration

Java > 9 is modular and doesn't expose all the internals by default. As a simple blanket fix, add this to your Java initiation

    --add-opens=java.rmi/sun.rmi.transport=ALL-UNNAMED --add-opens=java.base/java.util.concurrent=ALL-UNNAMED --add-opens=java.base/java.time=ALL-UNNAMED --add-opens=java.base/java.util=ALL-UNNAMED --add-opens=java.base/java.io=ALL-UNNAMED --add-opens=java.base/java.lang=ALL-UNNAMED

You can either add that to the `./gradlew bootRun` command or set the ENV `JDK_JAVA_OPTIONS` with that value

## Logs
- Fineract service generates logs and sends them to Rsyslog
- Rsyslog writes these logs to a file
- Awslog service reads this file and forwards the logs to cloudwatch

If logs aren't showing up in cloudwatch, check that the second step isn't failing because of the file permissions. Sometimes, they are set correctly but get overwritten by service restart [left over from earlier attempt to write to file directly]

The permission need to be set appropriately and system-d file edited to remove references to file and folder logs.
