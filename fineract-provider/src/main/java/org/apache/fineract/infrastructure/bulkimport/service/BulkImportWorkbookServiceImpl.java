...
public class BulkImportWorkbookServiceImpl implements BulkImportWorkbookService {
        ...
        try {
            if (entity != null && inputStream != null && fileDetail != null && locale != null && dateFormat != null) {
                ...
                if (entity.trim().equalsIgnoreCase(GlobalEntityType.CLIENTS_PERSON.toString())) {
                    ...
                    primaryColumn = 0;
                } else if (entity.trim().equalsIgnoreCase(GlobalEntityType.FSP_LOAN_TRANSACTIONS.toString())) {
                    entityType = GlobalEntityType.FSP_LOAN_TRANSACTIONS;
                    primaryColumn = 0;
                    ...
                    primaryColumn = 0;
                } else if (entity.trim().equalsIgnoreCase(GlobalEntityType.SELF_SERVICE_USERS.toString())) {
                    entityType = GlobalEntityType.SELF_SERVICE_USERS;
                    primaryColumn = 0;
                } else if (entity.trim().equalsIgnoreCase(GlobalEntityType.NEW_SAVINGS.toString())) {
                    entityType = GlobalEntityType.NEW_SAVINGS;
                    primaryColumn = 0;
                } else if (entity.trim().equalsIgnoreCase(GlobalEntityType.NEW_SHARES.toString())) {
                    entityType = GlobalEntityType.NEW_SHARES;
                    primaryColumn = 0;
                } else {
                    workbook.close();
    ...
    private Response buildResponse(DocumentData documentData) {
        ...
        FileData fileData = contentRepository.fetchFile(documentData);
        String fileName = "Output" + documentData.fileName();
        try {
            final Response.ResponseBuilder response = Response.ok(fileData.getByteSource().openBufferedStream());
