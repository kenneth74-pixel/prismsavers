...
public class SmsConfigUtils {
    ...
    public Map<String, Object> getMessageGateWayRequestURI(final String apiEndPoint, String apiQueueResourceDatas) {
        ...
        boolean emptyPath = messageGatewayConfigurationData.getEndPoint() == null
                || messageGatewayConfigurationData.getEndPoint().equals("/");
        // remove path elements from the endpoint because the URI Builder will deal with it
        String endPoint = emptyPath ? "" : messageGatewayConfigurationData.getEndPoint().replaceAll("/", "");
        if (emptyPath) {
            pathBuilder.append("{apiEndPoint}");
        } else {
            pathBuilder.append("{endPoint}/{apiEndPoint}");
        }
        UriBuilder builder = UriBuilder.fromPath(pathBuilder.toString()).host(messageGatewayConfigurationData.getHostName()).scheme("https")
                .port(messageGatewayConfigurationData.getPortNumber());
        URI uri = emptyPath ? builder.build(apiEndPoint) : builder.build(endPoint, apiEndPoint);
        HttpEntity<?> entity;
