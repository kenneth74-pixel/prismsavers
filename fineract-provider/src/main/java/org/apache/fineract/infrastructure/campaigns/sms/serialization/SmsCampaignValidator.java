...
public class SmsCampaignValidator {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(SmsCampaignValidator.class);
    ...
    public SmsCampaignValidator(FromJsonHelper fromApiJsonHelper, final DeviceRegistrationRepositoryWrapper deviceRegistrationRepository,
            ChargeRepository chargeRepository, PortfolioCommandSourceWritePlatformService commandsSourceWritePlatformService,
            SavingsAccountChargeRepository savingsAccountChargeRepository, SavingsAccountRepository savingsAccountRepository,
            LicenseUsageValidator licenseUsageValidator) {
    ...
    public void validateCreate(String json) {
        ...
        if (paramValueJsonObject.getAsJsonObject().get("isEmailCampaign") == null) {
    ...
    public boolean chargeSms(Long clientId) {
        final List<Charge> charges = chargeRepository.findAll();
        boolean isCharged = false;
        final List<SavingsAccount> savingsAccounts = this.savingsAccountRepository.findSavingAccountByClientId(clientId);

        if (savingsAccounts.size() > 0) {
            final SavingsAccount savingsAccount = savingsAccounts.get(0);
            for (final Charge charge : charges) {
                if (charge.isSmsCharge()) {
                    JSONObject json = new JSONObject();
                    json.put("locale", "en");
                    json.put("dateFormat", "dd MMMM yyyy");
                    json.put("amount", charge.getAmount());
                    json.put("dueDate", LocalDate.now(ZoneId.systemDefault()).format(DateTimeFormatter.ofLocalizedDate(FormatStyle.LONG)));
                    json.put("chargeId", charge.getId());

                    final CommandWrapper commandRequest = new CommandWrapperBuilder().createSavingsAccountCharge(savingsAccount.getId())
                            .withJson(String.valueOf(json)).build();

                    final CommandProcessingResult result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);

                    final Optional<SavingsAccountCharge> savingsAccountCharge = this.savingsAccountChargeRepository
                            .findById(result.resourceId());

                    if (savingsAccountCharge.isPresent()) {
                        SavingsAccountCharge sac = savingsAccountCharge.get();
                        json.remove("chargeId");

                        final CommandWrapper payChargeCommandRequest = new CommandWrapperBuilder()
                                .paySavingsAccountCharge(savingsAccount.getId(), sac.getId()).withJson(String.valueOf(json)).build();

                        final CommandProcessingResult payChargeResult = this.commandsSourceWritePlatformService
                                .logCommandSource(payChargeCommandRequest);
                        isCharged = true;
                    }
                }
            }
        ...
        if (!isCharged) {
            LOG.error("Sms has not been charged");
        }
        return isCharged;
    }
