...
public class TenantDataSourceFactory {
    ...
    public DataSource create(FineractPlatformTenant tenant) {
        ...
        synchronized (this.tenantToDataSourceMap) {
            DataSource candidate = this.tenantToDataSourceMap.get(connectionId);
            if (candidate != null) {
                ds = candidate;
            } else {
                ds = createNewDatasource(tenant);
                this.tenantToDataSourceMap.put(connectionId, ds);
            }
        }
        return ds;
    }

    private DataSource createNewDatasource(FineractPlatformTenant tenant) {
        ...
        // fixing these to lower values because we won't use this for long
