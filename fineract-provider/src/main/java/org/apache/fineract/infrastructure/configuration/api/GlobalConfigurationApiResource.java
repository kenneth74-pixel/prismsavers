...
public class GlobalConfigurationApiResource {
    ...
    @GET
    @Path("get-configuration/{configName}")
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    @Operation(summary = "Retrieve Global Configuration By Name", description = "Returns a global enable/disable configurations.\n" + "\n"
            + "Example Requests:\n" + "\n" + "configurations/HFB")
    public String retrieveConfigurationByName(@PathParam("configName") @Parameter(description = "configName") final String configName,
            @Context final UriInfo uriInfo) {
