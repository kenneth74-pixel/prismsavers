...
public class ClientWritePlatformServiceJpaRepositoryImpl implements ClientWritePlatformService {
    ...
    private final AppuserClientMapperReadService clientMapperService;
    private final AppUserReadPlatformService userReadService;
    ...
    public ClientWritePlatformServiceJpaRepositoryImpl(final PlatformSecurityContext context,
            ...
            final EntityDatatableChecksWritePlatformService entityDatatableChecksWritePlatformService, PlatformEmailService emailService,
            AppuserClientMapperReadService clientMapperService, AppUserReadPlatformService userReadService) {
    ...
    public CommandProcessingResult createClient(final JsonCommand command) {
        ...
        try {
            ...
            if (newClient.isAccountNumberRequiresAutoGeneration()) {
                ...
                newClient.updateAccountNo(accountNumberGenerator.generate(newClient, accountNumberFormat, true));
                this.clientRepository.save(newClient);
            }
    ...
    public CommandProcessingResult activateClient(final Long clientId, final JsonCommand command) {
        try {
            ...
            this.notifySelfServiceUserAboutActivation(client);
            return new CommandProcessingResultBuilder() //
                    .withCommandId(command.commandId()) //
                    .withOfficeId(client.officeId()) //
                    .withClientId(clientId) //
    ...
    /*
     * this should be handled by an event listener, but we have a short window to implement so interim position it is
     */
    private void notifySelfServiceUserAboutActivation(Client client) {
        Long selfServiceUserId = this.clientMapperService.getUserMappedToClient(client.getId());
        if (selfServiceUserId != null) {
            AppUserData user = this.userReadService.retrieveUser(selfServiceUserId);
            this.emailService.sendAccountActivated(client.getDisplayName(), user.getEmail(), user.username());
        }
    }
