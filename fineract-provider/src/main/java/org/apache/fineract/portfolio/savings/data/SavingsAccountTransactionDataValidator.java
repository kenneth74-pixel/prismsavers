...
public class SavingsAccountTransactionDataValidator {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(SavingsAccountTransactionDataValidator.class);

    @Autowired
    public SavingsAccountTransactionDataValidator(final FromJsonHelper fromApiJsonHelper,
            final ConfigurationDomainService configurationDomainService, PaymentValidator paymentValidator) {
    ...
    private void validatePaymentTypeDetails(final DataValidatorBuilder baseDataValidator, JsonElement element) {
        ...
        // validate payment channel if provided
        boolean hasPayment = this.fromApiJsonHelper.parameterExists(PaymentApiConstants.paymentChannelDetails, element);
        if (hasPayment) {
            JsonObject paymentSection = this.fromApiJsonHelper.extractJsonObjectNamed(PaymentApiConstants.paymentChannelDetails, element);
            this.paymentValidator.checkRequiredParameters(baseDataValidator, paymentSection);
        }
    ...
    private void throwExceptionIfValidationWarningsExist(final List<ApiParameterError> dataValidationErrors) {
        if (!dataValidationErrors.isEmpty()) {
            LOG.error("Data Validation Errors.", dataValidationErrors);
            //
