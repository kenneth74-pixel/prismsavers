...
public class AppuserClientMapperReadServiceImpl implements AppuserClientMapperReadService {
    ...
    @Override
    public Long getUserMappedToClient(final Long clientId) {
        final String sql = "select appuser_id from m_selfservice_user_client_mapping where client_id = " + clientId + " limit 1";
        return jdbcTemplate.query(sql, rs -> rs.next() ? rs.getLong("appuser_id") : null);
    }
