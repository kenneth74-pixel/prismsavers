...
public class SavingsAccountCharge extends AbstractPersistableCustom {
    ...
    /**
     * This is a hack function that checks if the chargeTime type is a withdraw fee, if true we skip the compareTo
     * function to return a non-zero(1) else we continue with the comparison This was brought in because the charges
     * with a withdraw fee have a due date that returns null
     */

    private int compareDates(Date dueDate, Date dueDate2, Integer chargeTimeType) {
        if (chargeTimeType.equals(ChargeTimeType.WITHDRAWAL_FEE.getValue())) {
            return 1; // returning 1 when either of the dueDates are null
        }
    ...
    public boolean equals(Object o) {
        ...
        return (penaltyCharge == that.penaltyCharge) && (paid == that.paid) && (waived == that.waived) && (status == that.status)
                ...
                && Objects.equals(chargeTime, that.chargeTime) && compareDates(dueDate, that.dueDate, that.chargeTime) == 0
                        ? Boolean.TRUE
