...
public class ClientRepositoryWrapper {
    ...
    public Optional<Long> findByName(String firstName, String lastName) {
        List<Client> clients = this.repository.findByNames(firstName.toLowerCase(), lastName.toLowerCase());
        if (clients.isEmpty()) {
            return Optional.empty();
        }
        return Optional.ofNullable(clients.get(0).getId());
    }
    ...
    public Long getActiveClientCount() {
        return this.repository.countActiveClients();
    }
}
