...
public class SelfSavingsDataValidator {
    ...
    public void validateHasPaymentIntegration(final String json) {
        if (StringUtils.isBlank(json)) {
            throw new InvalidJsonException();
        }
        final JsonElement element = this.fromApiJsonHelper.parse(json);
        if (!this.fromApiJsonHelper.parameterExists(PaymentApiConstants.paymentChannelDetails, element)) {
            throw new PaymentNotSupported("transaction requires payment if a self service user initiates");
        }
    }
