...
public interface AppUserRepository extends JpaRepository<AppUser, Long>, JpaSpecificationExecutor<AppUser>, PlatformUserRepository {
    ...
    List<AppUser> findAppUserByEmail(@Param("email") String email);

    @Query("Select appUser from AppUser appUser where appUser.token = :token")
    AppUser findAppUserByResetToken(@Param("token") String token);

    @Query("Select appUser from AppUser appUser where appUser.otp = :otp")
    AppUser findAppUserByOTP(@Param("otp") String otp);
}
