...
import org.apache.fineract.portfolio.shareaccounts.domain.ShareAccountTransactionRepository;
...
public class JournalEntryReadPlatformServiceImpl implements JournalEntryReadPlatformService {
    ...
    public JournalEntryReadPlatformServiceImpl(final RoutingDataSource dataSource,
            ...
            final OfficeReadPlatformService officeReadPlatformService, LoanTransactionRepository loanTransactionRepository,
            SavingsAccountTransactionRepository savingsAccountTransactionRepository,
            ShareAccountTransactionRepository shareAccountTransactionRepository,
    ...
    private final class GLJournalEntryMapper implements RowMapper<JournalEntryData> {
        ...
        public JournalEntryData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            if (associationParametersData.isTransactionDetailsRequired()) {
                ...
                if (entityType != null) {
                    transaction = Long.parseLong(transactionId.replaceAll("[^\\d]", ""));
                    if (transactionId.contains("SH")) {
                        shareId = findShareAccountId(transaction);
                    }
                    if (PortfolioAccountType.fromInt(entityTypeId).isLoanAccount()) {
                        loanId = findAccountId(transaction);
                    }
                    if (PortfolioAccountType.fromInt(entityTypeId).isSavingsAccount()) {
                        savingsId = findSavingsAccountId(transaction);
                    }
            ...
            return new JournalEntryData(id, officeId, officeName, glAccountName, glAccountId, glCode, accountType, transactionDate,
                    ...
                    transactionDetailData, currency, loanId, savingsId, shareId);
        }
    }
    ...
    private Long findAccountId(final Long transactionId) {
        Long loanId = null;
        Optional<LoanTransaction> loanTransaction = this.loanTransactionRepository.findById(transactionId);
        if (loanTransaction.isPresent()) {
            LoanTransaction trx = loanTransaction.get();
            Loan loan = trx.getLoan();
            loanId = loan.getId();
        }
        return loanId;
    }

    private Long findSavingsAccountId(final Long transactionId) {
        Long savingsId = null;
        Optional<SavingsAccountTransaction> savingsAccountTransaction = this.savingsAccountTransactionRepository.findById(transactionId);
        if (savingsAccountTransaction.isPresent()) {
            SavingsAccountTransaction trx = savingsAccountTransaction.get();
            SavingsAccount savingsAccount = trx.getSavingsAccount();
            savingsId = savingsAccount.getId();
        }
        return savingsId;
    }

    private Long findShareAccountId(final Long transactionId) {
        Long shareAccountId = null;
        Optional<ShareAccountTransaction> shareAccountTransaction = this.shareAccountTransactionRepository.findById(transactionId);
        if (shareAccountTransaction.isPresent()) {
            ShareAccountTransaction trx = shareAccountTransaction.get();
            ShareAccount shareAccount = trx.getShareAccount();
