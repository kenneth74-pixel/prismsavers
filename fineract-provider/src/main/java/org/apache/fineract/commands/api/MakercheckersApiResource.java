...
@Component
@Scope("singleton")
@Tag(name = "Maker Checker Functionality to provide a rudimentary approval process")
public class MakercheckersApiResource {
    ...
    private static final Set<String> RESPONSE_DATA_PARAMETERS = new HashSet<>(Arrays.asList("id", "actionName", "entityName", "resourceId",
            "subresourceId", "maker", "madeOnDate", "processingResult", "commandAsJson", "officeName", "groupLevelName", "groupName",
    ...
    public String approveMakerCheckerEntry(@PathParam("auditId") @Parameter(description = "auditId") final Long auditId,
        ...
        if (is(commandParam, "approve")) {
            Optional<CommandProcessingResult> approvalResult = this.writePlatformService.addApproval(auditId);
            result = approvalResult.orElseGet(() -> CommandProcessingResult.commandOnlyResult(auditId));
